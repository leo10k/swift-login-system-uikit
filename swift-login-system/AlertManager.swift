...
//  swift-login-system
//
//  Created by Leonardo Macedo on 16/01/24.
//
...
extension AlertManager {
    ...
    public static func showLogoutErrorAlert(on vc: UIViewController, with error: Error) {
        self.showBasicAlert(on: vc, title: "Log Out Signing In", message: "\(error.localizedDescription)")
    }
}
...
extension AlertManager {
    ...
    public static func showPasswordResetSent(on vc: UIViewController) {
        self.showBasicAlert(on: vc, title: "Passwod Reset Sent", message: nil)
    }
